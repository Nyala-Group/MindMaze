...
class Game(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.miniMapBG = (0,0,BLOCK_WIDTH*self.mazeWidth,BLOCK_HEIGHT*self.mazeHeight)
    ...
    def buildWalls(self,maze):
        for i in range(len(maze)):
            for j in range(len(maze)):
                if(maze[i][j] == 1):
                    self.walls.append((BLOCK_WIDTH*i, BLOCK_HEIGHT*j ,BLOCK_WIDTH, BLOCK_HEIGHT))
    ...
    def moveForward(self,direction):
        ...
        if(newPos[0] >= 0 and newPos[0] < self.mazeWidth and newPos[1] >= 0 and newPos[1] < self.mazeHeight):
    ...
    def play_step(self):
        ...
        # RIGHT
        if(self.dir == [1,0]):
            self.arrow_img = self.arrow_sheet[2] # show correct mini-map arrow direction
            
            index = self.imageTable[self.makeKey(self.checkRight())] # return index for image, based on direction we are looking
            index = self.toggleAltWall(index) # switch halloway image if appropriate
            self.maze_img = self.maze_sheet[index] # show chosen image
            
        # LEFT
        elif(self.dir == [-1,0]):
            self.arrow_img = self.arrow_sheet[3] # show correct mini-map arrow direction
            
            index = self.imageTable[self.makeKey(self.checkLeft())] # return index for image, based on direction we are looking
            index = self.toggleAltWall(index) # switch halloway image if appropriate
            self.maze_img = self.maze_sheet[index] # show chosen image
            
        # DOWN
        elif(self.dir == [0,1]):
            self.arrow_img = self.arrow_sheet[0] # show correct mini-map arrow direction
            
            index = self.imageTable[self.makeKey(self.checkDown())] # return index for image, based on direction we are looking
            index = self.toggleAltWall(index) # switch halloway image if appropriate
            self.maze_img = self.maze_sheet[index] # show chosen image
            
        # UP
        elif(self.dir == [0,-1]):
            self.arrow_img = self.arrow_sheet[1] # show correct mini-map arrow direction
            
            index = self.imageTable[self.makeKey(self.checkUp())] # return index for image, based on direction we are looking
            index = self.toggleAltWall(index) # switch halloway image if appropriate
            self.maze_img = self.maze_sheet[index] # show chosen image
    ...
    def ui(self):
        ...
        pygame.draw.rect(self.screen, WHITE, (self.miniMapBG[0], self.miniMapBG[1], self.miniMapBG[2], self.miniMapBG[3]))
